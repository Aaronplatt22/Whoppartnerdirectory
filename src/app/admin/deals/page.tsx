...
export default async function AdminDealsPage() {
  const deals = await prisma.deal.findMany({ include: { partner: true, cam: true }, orderBy: { updatedAt: "desc" } });
  const totalPipeline = deals.filter(d => !["Closed Won", "Closed Lost"].includes(d.stage)).reduce((s, d) => s + d.estimatedValue, 0);
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div><h1 className="text-2xl font-bold text-white">Deal Pipeline</h1>
        <p className="text-gray-400 text-sm mt-1">{"Active pipeline: $" + totalPipeline.toLocaleString()}</p></div>
      </div>
