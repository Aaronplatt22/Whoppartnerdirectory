...
export default function AMDealDetailPage() {
  ...
  return (
    <div>
      <button onClick={() => router.back()} className="text-sm text-gray-500 hover:text-gray-300 mb-4">← Back to Pipeline</button>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
        <h2 className="text-sm font-medium text-gray-400 mb-4">Move Deal Stage</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><span className="text-gray-500">Probability</span><span className="text-white">{deal.probability + "%"}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Processing</span><span className="text-white">{"$" + deal.monthlyProcessing.toLocaleString() + "/mo"}</span></div>
          </div>
        </div>

        <div className="lg:col-span-2 bg-gray-900 border border-gray-800 rounded-xl p-6">
          <h2 className="text-sm font-medium text-gray-400 mb-4">Activity</h2>
          ...
          <div className="space-y-3">
            {deal.activities?.map((a: any) => (
              <div key={a.id} className="flex gap-3">
                <div className={"w-2 h-2 rounded-full mt-1.5 shrink-0 " + (a.type === "stage_change" ? "bg-yellow-400" : "bg-blue-400")} />
                <div>
                  <p className="text-sm text-gray-300">{a.content}</p>
                  <p className="text-xs text-gray-600 mt-1">{a.author?.name + " • " + new Date(a.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
