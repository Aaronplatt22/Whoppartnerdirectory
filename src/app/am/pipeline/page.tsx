...
export default async function AMPipelinePage() {
  ...
  return (
    <div>
      ...
      <p className="text-gray-400 text-sm mt-1 mb-8">{"Active pipeline: $" + totalPipeline.toLocaleString() + " • Click to manage deals"}</p>
          ...
          return (
            <div key={stage} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
              ...
              <div className="space-y-2">{sd.map(deal => (
                <Link href={"/am/pipeline/" + deal.id} key={deal.id} className="block bg-gray-800/50 rounded-lg p-3 hover:bg-gray-800 transition-colors cursor-pointer">
