...
const nav = [
  { href: "/am", label: "Dashboard", icon: "📊" },
  { href: "/am/pipeline", label: "Deal Pipeline", icon: "💰" },
  { href: "/am/partners", label: "My Partners", icon: "🤝" },
  { href: "/am/notifications", label: "Notifications", icon: "🔔" },
];

export default function AMSidebar({ userName }: { userName: string }) {
  const pathname = usePathname();
  return (
    <aside className="w-64 bg-gray-900 border-r border-gray-800 flex flex-col">
      <div className="p-6 border-b border-gray-800">
        <h1 className="text-xl font-bold text-white">Whop Partner Hub</h1>
        <p className="text-sm text-gray-400 mt-1">CAM Portal</p>
      </div>
      <div className="px-6 py-4 border-b border-gray-800">
        <p className="text-sm text-white font-medium">{userName}</p>
        <p className="text-xs text-gray-500">Channel Account Manager</p>
      </div>
      <nav className="flex-1 p-4 space-y-1">
        {nav.map((item) => {
          const active = pathname === item.href || (item.href !== "/am" && pathname.startsWith(item.href));
          return (
